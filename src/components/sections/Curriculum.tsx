...
export default function Curriculum() {
  return (
    <section id="curriculum" className="bg-brand-light-bg py-16 md:py-28 px-5 sm:px-6 md:px-12 border-t border-slate-200/85">
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold text-brand-navy tracking-tight mb-6 max-w-4xl mx-auto leading-tight">
            Learn AI-Powered <span className="text-brand-primary">Trading & Analysis</span>
          </h2>
            ...
            return (
              <motion.div 
                ...
                className={`border rounded-[32px] p-6 sm:p-8 flex flex-col hover:shadow-xl transition-all duration-300 h-full group bg-white ${
                  isPremium ? 'border-brand-primary shadow-md ring-2 ring-brand-primary/10' : 'border-slate-200/80 shadow-sm'
                }`}
              >
                {/* Badges & Meta Info */}
                <div className="flex justify-between items-center mb-6">
                  <span className={`text-xs font-bold px-3 py-1.5 rounded-full uppercase tracking-wider ${
                    isPremium ? 'bg-brand-primary/10 text-brand-primary' : 'bg-brand-secondary/10 text-brand-secondary-hover'
                  }`}>
                    {course.badge}
                  </span>
                  <span className="text-lg font-extrabold text-brand-navy">{course.fee}</span>
                </div>

                <h3 className={`text-xl sm:text-2xl font-extrabold text-brand-navy mb-4 leading-snug tracking-tight transition-colors ${
                ...
                <div className="mb-8 bg-slate-50 rounded-2xl p-5 border border-slate-200/85">
                  ...
                  <ul className="space-y-3">
                    {course.modules.slice(0, 3).map((mod, idx) => (
                      <li key={idx} className="flex items-center gap-3">
                        ...
                        <span className="text-slate-600 font-medium text-[13.5px] line-clamp-1">{mod.title}</span>
                      </li>
                    ))}
                    {course.modules.length > 3 && (
                      <li className="text-[12.5px] text-slate-500 font-semibold pl-8">
                ...
                <div className="space-y-3 mt-auto">
                  <a href={`#/course-details/${course.id}`} className="block w-full">
                    <motion.button 
                      ...
                      whileTap={{ scale: 0.98 }} 
                      className={`w-full font-bold py-4 rounded-xl transition-all flex justify-center items-center gap-2 uppercase cursor-pointer text-sm border-0 ${
                        isPremium 
                          ? 'bg-gradient-to-r from-brand-primary-grad-start to-brand-primary-grad-end text-brand-navy hover:brightness-110 shadow-lg shadow-brand-primary/15' 
                          : 'bg-brand-navy hover:bg-slate-900 text-white shadow-md'
                      }`}
                    >
                      View Full Syllabus
