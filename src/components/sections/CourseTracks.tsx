...
const themeStyles = {
  ...
  indigo: {
    container: 'bg-white border-[3px] border-brand-primary shadow-xl z-10 lg:-translate-y-4 lg:scale-105',
  ...
  orange: {
    container: 'bg-white border-slate-200 hover:border-brand-primary/45 shadow-sm',
    badge: 'bg-brand-primary/10 text-brand-primary border-brand-primary/20',
    bullet: 'bg-brand-primary',
...
export default function CourseTracks() {
  ...
  return (
    <section id="courses" className="py-16 md:py-32 px-5 sm:px-6 md:px-12 bg-white text-center border-t border-slate-200/80">
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <h2 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold text-brand-navy tracking-tight mb-8 max-w-4xl mx-auto leading-tight">
            Choose your track – <span className="text-brand-primary">stock market & trading courses</span> for every goal
          </h2>
        </motion.div>
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-wrap gap-3 mb-8">
                 ...
                 <h3 className="text-xl sm:text-2xl lg:text-[26px] font-extrabold text-brand-navy mb-6 sm:mb-8 leading-snug tracking-tight">
                  {course.title}
                </h3>
                ...
                <div className="space-y-4 mb-8">
                  <div className="flex gap-4">
                    ...
                    <span className="font-semibold text-slate-700 text-[15px]">{course.duration}</span>
                  </div>
                  <div className="flex gap-4">
                    <span className="w-24 text-slate-500 font-bold text-xs tracking-widest uppercase mt-0.5">Format</span>
                    <span className="font-semibold text-slate-700 text-[15px]">{course.format}</span>
                  </div>
                  <div className="flex gap-4">
                    <span className="w-24 text-slate-500 font-bold text-xs tracking-widest uppercase mt-0.5">Schedule</span>
                    <span className="font-semibold text-slate-700 text-[15px]">{course.schedule}</span>
                  </div>
                </div>
                  ...
                  <ul className="space-y-3">
                    {course.topics.map((topic, i) => (
                      <li key={i} className="flex items-start gap-3">
                        ...
                        <span className="text-slate-600 font-medium text-[15px]">{topic}</span>
                      </li>
                    ))}
                    
                    {course.idealAudience && (
                      <li className={`flex items-start gap-3 mt-4 text-[15px] font-medium text-slate-600 p-3 rounded-xl border shadow-sm ${styles.extraItem}`}>
                    ...
                    {course.bonus && (
                      <li className={`flex items-start gap-3 mt-3 text-[15px] font-medium text-slate-700 p-3 rounded-xl border shadow-sm bg-brand-success/10 border-brand-success/25`}>
                ...
                <div className="mt-auto space-y-4 pt-6 border-t border-slate-100">
                  <div className="bg-slate-50 rounded-2xl p-5 border border-slate-200/80 shadow-sm">
                  ...
                  <div className={`rounded-2xl p-5 border ${styles.batchBg}`}>
                    ...
                    {course.remainingSeatsText && (
                      <div className="inline-flex items-center gap-2 bg-white/50 rounded-lg px-3 py-2 text-xs font-bold text-slate-600 shadow-sm border border-slate-200/60 mt-2">
                  ...
                  <a href="#final-cta" className="block w-full">
                    <motion.button 
                      ...
                      whileTap={{ scale: 0.98 }} 
                      className={`w-full font-bold py-4 rounded-xl shadow-md border-0 transition-colors flex justify-center items-center gap-2 mt-2 uppercase cursor-pointer ${styles.primaryButton}`}
                    >
                  ...
                  <a href={`https://wa.me/918861756040?text=Hi%2C%20I%20want%20to%20download%20the%20syllabus%20for%20${encodeURIComponent(course.title)}.`} target="_blank" rel="noopener noreferrer" className="block w-full">
                    <motion.button 
                      whileHover={{ scale: 1.02 }} 
                      whileTap={{ scale: 0.98 }} 
                      className="w-full bg-white border border-slate-200 text-brand-navy font-bold py-4 rounded-xl hover:bg-slate-50 transition-colors uppercase text-sm shadow-sm cursor-pointer"
                    >
        ...
        <div className="mt-16 flex justify-center">
          <a href="#/all-courses">
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              className="px-8 py-5 rounded-2xl bg-gradient-to-r from-brand-primary-grad-start to-brand-primary-grad-end text-brand-navy font-extrabold text-lg tracking-wide hover:brightness-110 hover:shadow-lg hover:shadow-brand-primary/15 transition-all flex items-center gap-3 border-0 uppercase cursor-pointer"
            >
