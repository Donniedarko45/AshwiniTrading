...
const themeStyles = {
  ...
  indigo: {
    container: 'bg-brand-bg-card border-[3px] border-brand-primary shadow-2xl z-10 lg:-translate-y-4 lg:scale-105',
  ...
  orange: {
    container: 'bg-brand-bg-card/50 border-white/10 hover:border-brand-primary/35',
    badge: 'bg-brand-primary/10 text-brand-primary-hover border-brand-primary/20',
    bullet: 'bg-brand-primary',
...
export default function CourseTracks() {
  ...
  return (
    <section id="courses" className="py-16 md:py-32 px-5 sm:px-6 md:px-12 bg-brand-bg-dark text-center border-t border-white/5">
              ...
              >
                ...
                <div className="space-y-4 mb-8">
                  <div className="flex gap-4">
                    ...
                    <span className="font-semibold text-slate-200 text-[15px]">{course.duration}</span>
                  </div>
                  <div className="flex gap-4">
                    <span className="w-24 text-slate-500 font-bold text-xs tracking-widest uppercase mt-0.5">Format</span>
                    <span className="font-semibold text-slate-200 text-[15px]">{course.format}</span>
                  </div>
                  <div className="flex gap-4">
                    <span className="w-24 text-slate-500 font-bold text-xs tracking-widest uppercase mt-0.5">Schedule</span>
                    <span className="font-semibold text-slate-200 text-[15px]">{course.schedule}</span>
                  </div>
                </div>
                  ...
                  <ul className="space-y-3">
                    {course.topics.map((topic, i) => (
                      <li key={i} className="flex items-start gap-3">
                        ...
                        <span className="text-slate-300 font-medium text-[15px]">{topic}</span>
                      </li>
                    ))}
                    
                    {course.idealAudience && (
                      <li className={`flex items-start gap-3 mt-4 text-[15px] font-medium text-slate-300 p-3 rounded-xl border shadow-sm ${styles.extraItem}`}>
                    ...
                    {course.bonus && (
                      <li className={`flex items-start gap-3 mt-3 text-[15px] font-medium text-slate-200 p-3 rounded-xl border shadow-sm bg-brand-success/15 border-brand-success/20`}>
                ...
                <div className="mt-auto space-y-4 pt-6 border-t border-white/10">
                  <div className="bg-white/5 rounded-2xl p-5 border border-white/10 shadow-sm">
                    ...
                    <div className="text-xs sm:text-[11px] font-bold text-slate-400 uppercase tracking-wider">
                  ...
                  <div className={`rounded-2xl p-5 border ${styles.batchBg}`}>
                    ...
                    {course.remainingSeatsText && (
                      <div className="inline-flex items-center gap-2 bg-white/5 rounded-lg px-3 py-2 text-xs font-bold text-slate-300 shadow-sm border border-white/10 mt-2">
                  ...
                  <a href={`https://wa.me/919845961990?text=Hi%2C%20I%20want%20to%20download%20the%20syllabus%20for%20${encodeURIComponent(course.title)}.`} target="_blank" rel="noopener noreferrer" className="block w-full">
                    <motion.button 
                      whileHover={{ scale: 1.02 }} 
                      whileTap={{ scale: 0.98 }} 
                      className="w-full bg-white/5 border border-white/10 text-white font-bold py-4 rounded-xl hover:bg-white/10 transition-colors uppercase text-sm"
                    >
