...
export default function CoursesWithAI() {
  ...
  return (
    <section id="courses-ai" className="relative py-24 md:py-32 bg-gradient-to-b from-brand-bg-dark to-brand-bg-deep overflow-hidden px-5 sm:px-6 md:px-12 border-t border-b border-white/5">
      ...
      {/* Decorative glows */}
      <div className="absolute top-1/4 left-[-10%] w-[600px] h-[600px] bg-brand-glow/15 rounded-full blur-[130px] pointer-events-none" />
      <div className="absolute bottom-1/4 right-[-10%] w-[600px] h-[600px] bg-brand-glow/20 rounded-full blur-[130px] pointer-events-none" />
          ...
          {courses.map((course) => {
            ...
            return (
              <motion.div
                key={course.id}
                variants={scaleUp}
                className="group relative rounded-[32px] p-6 sm:p-8 md:p-10 bg-brand-bg-card/40 border border-white/5 backdrop-blur-xl transition-all duration-500 hover:-translate-y-1.5 flex flex-col justify-between overflow-hidden shadow-2xl"
              >
                ...
                <div className="relative z-10">
                  ...
                  <div className="space-y-4 mb-8 sm:mb-10">
                    <div className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-2">What you will learn:</div>
                    <div className="grid sm:grid-cols-2 gap-3">
                      {course.points.map((point, idx) => (
                        <div key={idx} className="flex items-start gap-2.5">
                          ...
                          <span className="text-slate-300 text-[14px] font-medium leading-tight">{point}</span>
                        </div>
                      ))}
                    </div>
                  </div>
