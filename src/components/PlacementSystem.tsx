  ...
  return (
    <section className="bg-gradient-to-br from-[#EAF3FF] to-[#D5F5E3] py-24 px-6 relative overflow-hidden">
        <div className="max-w-7xl mx-auto">
            <div className="text-blue-600 font-bold text-sm tracking-widest uppercase mb-4">THE PLACEMENT SYSTEM</div>
            ...
            <div className="relative">
                ...
                <div className="absolute top-4 right-4 w-[20%] h-0.5 border-t-2 border-dashed border-emerald-400 hidden lg:block" />

                <div className="grid lg:grid-cols-5 gap-6">
                    {steps.map((step, i) => (
                        <div key={i} className="relative pt-8 lg:pt-10">
                            {/* Dots */}
                            <div className="hidden lg:flex absolute top-2.5 left-1/2 -translate-x-1/2 w-4 h-4 rounded-full bg-white border-[3px] border-blue-500 z-10" 
                                style={{ borderColor: step.isGreen ? '#10B981' : '#3B82F6', width: step.isGreen ? '20px' : '16px', height: step.isGreen ? '20px' : '16px', top: step.isGreen ? '6px' : '10px' }} 
                            />
                            ...
                            <div className={`bg-white rounded-2xl p-6 shadow-sm border ${step.isGreen ? 'border-emerald-200 h-full' : 'border-white h-full'} hover:shadow-md transition-shadow`}>
                                <div className={`text-xs font-extrabold tracking-widest uppercase mb-3 ${step.isGreen ? 'text-slate-900 text-lg' : 'text-blue-600'}`}>
                                    {step.month}
                                </div>
                                {step.title && (
